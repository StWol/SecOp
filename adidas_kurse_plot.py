...
sql1 = """SELECT `neues_kursziel`,  `zieldatum`
    ...
    WHERE `zeithorizont`>0 AND `neues_kursziel`>0 AND `unternehmen` =4 AND `analyst` = `analyst`.`id` AND `analyst`.`analystenhaus`=`analystenhaus`.`id`AND datum> '2010-01-01' """
sql2 = """SELECT avg,datum FROM unternehmen_avg WHERE unternehmen = 4 ORDER BY datum"""
